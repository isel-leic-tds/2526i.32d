...
fun Board.getResult(): BoardResult =
    when {
        hasWon(Marker.CIRCLE) -> BoardResult.HasWinner(Marker.CIRCLE)
        hasWon(Marker.CROSS) -> BoardResult.HasWinner(Marker.CROSS)
        toMovesList().all { it != null } -> BoardResult.Tied()
        else -> BoardResult.OnGoing()
    }